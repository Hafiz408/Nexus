...
Lifespan I/O (init_db, init_pgvector_table) is also patched so tests run
without a real Postgres connection or SQLite file.
"""
import json
...
def test_unindexed_repo_returns_400(client, monkeypatch):
    ...
    response = client.post("/query", json={"question": "What is bar?", "repo_path": "/repo"})
...
def test_happy_path_yields_token_events(client, monkeypatch, sample_node, sample_stats):
    ...
    body = _read_stream(client, {"question": "What is bar?", "repo_path": "/repo"})
...
def test_error_event_on_retrieval_failure(client, monkeypatch):
    ...
    def _bad_retrieve(question, repo_path, G, max_nodes, hop_depth):
