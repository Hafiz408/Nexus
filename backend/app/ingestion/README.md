...
POST /index
    ...
    │               emit CodeNode + raw CALLS/IMPORTS/CLASS_CONTAINS edge tuples
...
| `ast_parser.py` | tree-sitter for Python + TypeScript; extracts signature, docstring, body preview, complexity; emits `CLASS_CONTAINS` edges linking each class to its methods |
| `graph_builder.py` | Resolves raw edge tuples by name/module registry; builds DiGraph; runs PageRank; handles `CLASS_CONTAINS` edge type |
...
SQLite (.nexus/graph.db)
  ...
  graph_edges     source · target · repo_path · edge_type  (CALLS · IMPORTS · CLASS_CONTAINS)
