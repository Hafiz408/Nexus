...
Question
  ...
  ├─ PPR expand: Personalized PageRank seeded from RRF scores; traverses CALLS + CLASS_CONTAINS
  │   edges (IMPORTS excluded); returns top-30 non-seed neighbors by PPR score
  ├─ Combine pool: seeds overwrite neighbors on overlap + test-file ×0.5 penalty
  ├─ Cross-encoder rerank (default on): ms-marco-MiniLM-L-6-v2 jointly scores
  │   (query, node_context) pairs over top 2×N; falls back silently on error
  ├─ Hybrid CE floor: always keep top-3; drop candidates >4.0 logit-units below best
  ├─ Full-body expansion: top-5 CE-ranked nodes fetch complete source (line_start→line_end)
